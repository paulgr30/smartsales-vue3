...
const onGetByCriteria = async () => {
  await get(
    "http://localhost/smartsales/smartsales-laravel10/public/api/identitydocuments/bycriteria",
    {
...
const onToggleStatus = async (item) => {
  await patch(
    `http://localhost/smartsales/smartsales-laravel10/public/api/identitydocuments/${item.id}`,
    item
  );
};
          ...
          <div class="col-12 q-mb-md">
            <form autocomplete="off" @submit.prevent="onGetByCriteria()">
              <q-input
                dense
                type="search"
                placeholder="Ingrese nombre del documento de identidad"
                v-model="searchBox"
              >
